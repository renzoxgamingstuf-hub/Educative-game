...
public class GameManager : MonoBehaviour
{
    ...
    // PlayerPrefs key for storing the highest finished level
    ...
    void ResetGame()
    {
        // On failure, step back one level rather than resetting to level 1
        operations.Clear();
        level = Mathf.Max(1, level - 1);
        sequenceLength = 3 + (level - 1);
        UpdateLevelDisplay();
        ShowFeedback("You fell back to Level " + level + ".");
    ...
    void SaveProgress()
    {
        int maxReached = PlayerPrefs.GetInt(PREF_MAX_LEVEL, 1);
        if (level > maxReached)
        {
            PlayerPrefs.SetInt(PREF_MAX_LEVEL, level);
            PlayerPrefs.Save();
        }
    }
    ...
    void LoadProgress()
    {
        int savedMax = PlayerPrefs.GetInt(PREF_MAX_LEVEL, 1);
        // Start the player one level below the last finished level (as requested)
        int startLevel = Mathf.Max(1, savedMax - 1);
        level = startLevel;
        // Make sequenceLength align with level: base 3 plus (level-1)
        sequenceLength = 3 + (level - 1);
    }

    public void ResetProgress()
    {
        PlayerPrefs.DeleteKey(PREF_MAX_LEVEL);
        level = 1;
        sequenceLength = 3;
    }
